...
<style type="text/css">
    ...
    .buttons{margin-top:5px;margin-right:50px;margin-left:20px;float:right;margin-top:initial;}
    .now {margin-top:20px;margin-bottom:20px;}
    .myForm {font-weight: bold;margin-left:0px;}
    .tgx {font-weight: bold;border-collapse:collapse;border-spacing:0;margin-left:0px;color:#FFFFFF;margin-left:0px;margin-right:75px;margin-bottom:30px;}
    .tgx td{font-weight: bold;border-color:black;border-style:solid;border-width:1px;font-family:Arial, sans-serif;font-size:14px;
    overflow:hidden;padding:5px 2px;word-break:normal;}
    .tgx .fig{font-weight: bold;color:#000000;background-color:#FFFFFF;padding-left:20px;padding-right:12px;}
    .tgx th{font-weight: bold;border-color:red;border-style:solid;border-width:2px;font-family:Arial, sans-serif;font-size:14px;
    font-weight:normal;overflow:hidden;padding:5px 40px;word-break:normal;}

    .tg  {border-collapse:collapse;border-spacing:0;margin-left:20px;color:#FFFFFF;margin-left:20px;margin-right:75px;}
    .tg td{border-color:black;border-style:solid;border-width:0px;font-family:Arial, sans-serif;font-size:12px;
    overflow:hidden;padding:5px 5px;word-break:normal;}
    // .tg .tg-val:hover{background-color:#fca903;color:black;}
...
echo '<form class="myForm" name="myForm"><label> Group#1 : </label>
...
echo '<form class="myForm" name="myForm2"><label> Group#2 : </label>
...
echo '<form class="myForm" name="myForm3"><label> Group#3 : </label>';
...
echo '<table class="tg" style="background-color:#242424;display:none;">';
echo '<tr>
...
echo '<div class="now6Month" style="display:none;" value='.$now_6month.'></div>';
...
$html_text = '<script>
    ...
    function calcFigure2(code,code2,code3){
        ...
        for(let i = 0; i < games.length; i++){ 
            ...
            if(isNaN(roiDay)){roiDay = 0;}
            var roiWeek = sumWeek / rWeek * 100;
            if(isNaN(roiWeek)){roiWeek = 0;}
            var roiMonth = sumMonth / rMonth * 100;
            if(isNaN(roiMonth)){roiMonth = 0;}
            var roiMonth6 = sumMonth6 / rMonth6 * 100;
            if(isNaN(roiMonth6)){roiMonth6 = 0;}
            var roiYear = sumYear / rYear * 100;
            if(isNaN(roiYear)){roiYear = 0;}
            var roiAll = sumAll / rAll * 100;
            if(isNaN(roiAll)){roiAll = 0;}

            var redcolor="#f77474";
            var bluecolor="#8cb6ff";
            document.getElementById("pld"+games[i]).innerHTML = "$  "+sumDay.toFixed(2);
            if(sumDay<0){document.getElementById("pld"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("pld"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("plw"+games[i]).innerHTML = "$  "+sumWeek.toFixed(2);
            if(sumWeek<0){document.getElementById("plw"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("plw"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("plm"+games[i]).innerHTML = "$  "+sumMonth.toFixed(2);
            if(sumMonth<0){document.getElementById("plm"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("plm"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("pl6m"+games[i]).innerHTML = "$  "+sumMonth6.toFixed(2);
            if(sumMonth6<0){document.getElementById("pl6m"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("pl6m"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("ply"+games[i]).innerHTML = "$  "+sumYear.toFixed(2);
            if(sumYear<0){document.getElementById("ply"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("ply"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("pla"+games[i]).innerHTML = "$  "+sumAll.toFixed(2);
            if(sumAll<0){document.getElementById("pla"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("pla"+games[i]).style.backgroundColor=bluecolor;}

            document.getElementById("pldr"+games[i]).innerHTML = roiDay.toFixed(2)+" %";
            if(roiDay<0){document.getElementById("pldr"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("pldr"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("plwr"+games[i]).innerHTML = roiWeek.toFixed(2)+" %";
            if(roiWeek<0){document.getElementById("plwr"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("plwr"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("plmr"+games[i]).innerHTML = roiMonth.toFixed(2)+" %";
            if(roiMonth<0){document.getElementById("plmr"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("plmr"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("pl6mr"+games[i]).innerHTML = roiMonth6.toFixed(2)+" %";
            if(roiMonth6<0){document.getElementById("pl6mr"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("pl6mr"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("plyr"+games[i]).innerHTML = roiYear.toFixed(2)+" %";
            if(roiYear<0){document.getElementById("plyr"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("plyr"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("plar"+games[i]).innerHTML = roiAll.toFixed(2)+" %";
            if(roiAll<0){document.getElementById("plar"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("plar"+games[i]).style.backgroundColor=bluecolor;}
            
            redcolor = "#faa866";
            bluecolor = "#faa866";
            document.getElementById("rd"+games[i]).innerHTML = "$  "+rDay.toFixed(2);
            if(rDay<0){document.getElementById("rd"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("rd"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("rw"+games[i]).innerHTML = "$  "+rWeek.toFixed(2);
            if(rWeek<0){document.getElementById("rw"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("rw"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("rm"+games[i]).innerHTML = "$  "+rMonth.toFixed(2);
            if(rMonth<0){document.getElementById("rm"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("rm"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("r6m"+games[i]).innerHTML = "$  "+rMonth6.toFixed(2);
            if(rMonth6<0){document.getElementById("r6m"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("r6m"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("ry"+games[i]).innerHTML = "$  "+rYear.toFixed(2);
            if(rYear<0){document.getElementById("ry"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("ry"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("ra"+games[i]).innerHTML = "$  "+rAll.toFixed(2);
            if(rAll<0){document.getElementById("ra"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("ra"+games[i]).style.backgroundColor=bluecolor;}
        }
        var totRoiDay = totDay / totRDay * 100;
        if(isNaN(totRoiDay)){totRoiDay = 0;}
        var totRoiWeek = totWeek / totRWeek * 100;
        if(isNaN(totRoiWeek)){totRoiWeek = 0;}
        var totRoiMonth = totMonth / totRMonth * 100;
        if(isNaN(totRoiMonth)){totRoiMonth = 0;}
        var totRoiMonth6 = totMonth6 / totRMonth6 * 100;
        if(isNaN(totRoiMonth6)){totRoiMonth6 = 0;}
        var totRoiYear = totYear / totRYear * 100;
        if(isNaN(totRoiYear)){totRoiYear = 0;}
        var totRoiAll = totAll / totRAll * 100;
        if(isNaN(totRoiAll)){totRoiAll = 0;}

        var redcolor="#f77474";
        var bluecolor="#8cb6ff";
        document.getElementById("pld"+"x").innerHTML = "$  "+totDay.toFixed(2);
        if(totDay<0){document.getElementById("pld"+"x").style.backgroundColor=redcolor;}else{document.getElementById("pld"+"x").style.backgroundColor=bluecolor;}
        document.getElementById("plw"+"x").innerHTML = "$  "+totWeek.toFixed(2);
        if(totWeek<0){document.getElementById("plw"+"x").style.backgroundColor=redcolor;}else{document.getElementById("plw"+"x").style.backgroundColor=bluecolor;}
        document.getElementById("plm"+"x").innerHTML = "$  "+totMonth.toFixed(2);
        if(totMonth<0){document.getElementById("plm"+"x").style.backgroundColor=redcolor;}else{document.getElementById("plm"+"x").style.backgroundColor=bluecolor;}
        document.getElementById("pl6m"+"x").innerHTML = "$  "+totMonth6.toFixed(2);
        if(totMonth6<0){document.getElementById("pl6m"+"x").style.backgroundColor=redcolor;}else{document.getElementById("pl6m"+"x").style.backgroundColor=bluecolor;}
        document.getElementById("ply"+"x").innerHTML = "$  "+totYear.toFixed(2);
        if(totYear<0){document.getElementById("ply"+"x").style.backgroundColor=redcolor;}else{document.getElementById("ply"+"x").style.backgroundColor=bluecolor;}
        document.getElementById("pla"+"x").innerHTML = "$  "+totAll.toFixed(2);
        if(totAll<0){document.getElementById("pla"+"x").style.backgroundColor=redcolor;}else{document.getElementById("pla"+"x").style.backgroundColor=bluecolor;}

        document.getElementById("pldr"+"x").innerHTML = totRoiDay.toFixed(2)+" %";
        if(totRoiDay<0){document.getElementById("pldr"+"x").style.backgroundColor=redcolor;}else{document.getElementById("pldr"+"x").style.backgroundColor=bluecolor;}
        document.getElementById("plwr"+"x").innerHTML = totRoiWeek.toFixed(2)+" %";
        if(totRoiWeek<0){document.getElementById("plwr"+"x").style.backgroundColor=redcolor;}else{document.getElementById("plwr"+"x").style.backgroundColor=bluecolor;}
        document.getElementById("plmr"+"x").innerHTML = totRoiMonth.toFixed(2)+" %";
        if(totRoiMonth<0){document.getElementById("plmr"+"x").style.backgroundColor=redcolor;}else{document.getElementById("plmr"+"x").style.backgroundColor=bluecolor;}
        document.getElementById("pl6mr"+"x").innerHTML = totRoiMonth6.toFixed(2)+" %";
        if(totRoiMonth6<0){document.getElementById("pl6mr"+"x").style.backgroundColor=redcolor;}else{document.getElementById("pl6mr"+"x").style.backgroundColor=bluecolor;}
        document.getElementById("plyr"+"x").innerHTML = totRoiYear.toFixed(2)+" %";
        if(totRoiYear<0){document.getElementById("plyr"+"x").style.backgroundColor=redcolor;}else{document.getElementById("plyr"+"x").style.backgroundColor=bluecolor;}
        document.getElementById("plar"+"x").innerHTML = totRoiAll.toFixed(2)+" %";
        if(totRoiAll<0){document.getElementById("plar"+"x").style.backgroundColor=redcolor;}else{document.getElementById("plar"+"x").style.backgroundColor=bluecolor;}
        
        redcolor = "#faa866";
        bluecolor = "#faa866";
        document.getElementById("rd"+"x").innerHTML = "$  "+totRDay.toFixed(2);
        if(totRDay<0){document.getElementById("rd"+"x").style.backgroundColor=redcolor;}else{document.getElementById("rd"+"x").style.backgroundColor=bluecolor;}
        document.getElementById("rw"+"x").innerHTML = "$  "+totRWeek.toFixed(2);
        if(totRWeek<0){document.getElementById("rw"+"x").style.backgroundColor=redcolor;}else{document.getElementById("rw"+"x").style.backgroundColor=bluecolor;}
        document.getElementById("rm"+"x").innerHTML = "$  "+totRMonth.toFixed(2);
        if(totRMonth<0){document.getElementById("rm"+"x").style.backgroundColor=redcolor;}else{document.getElementById("rm"+"x").style.backgroundColor=bluecolor;}
        document.getElementById("r6m"+"x").innerHTML = "$  "+totRMonth6.toFixed(2);
        if(totRMonth6<0){document.getElementById("r6m"+"x").style.backgroundColor=redcolor;}else{document.getElementById("r6m"+"x").style.backgroundColor=bluecolor;}
        document.getElementById("ry"+"x").innerHTML = "$  "+totRYear.toFixed(2);
        if(totRYear<0){document.getElementById("ry"+"x").style.backgroundColor=redcolor;}else{document.getElementById("ry"+"x").style.backgroundColor=bluecolor;}
        document.getElementById("ra"+"x").innerHTML = "$  "+totRAll.toFixed(2);
        if(totRAll<0){document.getElementById("ra"+"x").style.backgroundColor=redcolor;}else{document.getElementById("ra"+"x").style.backgroundColor=bluecolor;}
    }
    ...
    function calcFigure(code,code2,code3){
        ...
        for(let i = 0; i < games.length; i++){ 
            ...
            var gameType = "";
            if(games[i]=="s"){
                gameType = "Soccer";}else if(games[i]=="b"){
                    gameType = "Baseball";}else if(games[i]=="t"){
                        gameType = "Tennis";}else if(games[i]=="bb"){
                            gameType = "Basketball";}else if(games[i]=="f"){
                                gameType = "Football";}else if(games[i]=="fi"){
                                    gameType = "Fighting";}else if(games[i]=="h"){
                                        gameType = "Hockey";}else if(games[i]=="g"){
                                            gameType = "Golf";}
            var count=0;
            for (let i=1;i<=table;i++){
                var data_time = document.getElementsByClassName("row-"+i)[0].getAttribute("data-time");
            ...
            if(wlDay<=50){document.getElementById("wrdr"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("wrdr"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("wrwr"+games[i]).innerHTML = wlWeek.toFixed(2);
            document.getElementById("wrw"+games[i]).innerHTML = wWeek+" - "+lWeek;
            if(wlWeek<=50){document.getElementById("wrwr"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("wrwr"+games[i]).style.backgroundColor=bluecolor;}
            ...
            if(wlMonth<=50){document.getElementById("wrmr"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("wrmr"+games[i]).style.backgroundColor=bluecolor;}
            ...
            if(wlMonth6<=50){document.getElementById("wr6mr"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("wr6mr"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("wryr"+games[i]).innerHTML = wlYear.toFixed(2);
            document.getElementById("wry"+games[i]).innerHTML = wYear+" - "+lYear;
            if(wlYear<=50){document.getElementById("wryr"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("wryr"+games[i]).style.backgroundColor=bluecolor;}
            document.getElementById("wrar"+games[i]).innerHTML = wlAll.toFixed(2);
            document.getElementById("wra"+games[i]).innerHTML = wAll+" - "+lAll;
            if(wlAll<=50){document.getElementById("wrar"+games[i]).style.backgroundColor=redcolor;}else{document.getElementById("wrar"+games[i]).style.backgroundColor=bluecolor;}
